/** linuxdo-notify background — activity heartbeat + open action.
 ...
 * plugin.storage so settings page can forward it when open.
 */
...
window.addEventListener('catrace:plugin-event-resolved', (ev) => {
  ...
  if (!detail || detail.kind !== 'linuxdo-notify') return
  if (detail.actionId === 'open') {
    ...
    if (url && plugin.shell && typeof plugin.shell.openExternal === 'function') {
      plugin.shell.openExternal(url).catch((e) => {
        console.warn('[linuxdo-notify] openExternal failed', e)
      })
    }
  }
})

...
**门控**：短文本（≤3 行）没有展开可言——无 hover 样式、无 title，点击不拦截、直接冒泡给卡片的「前往会话」。`bodyClamped`（mounted/updated 实测 scrollHeight）是 hover 与点击的总开关。
...
- **「前往会话」深色实心按钮常驻右下角**（用户明确选了不 hover 化；点击冒泡到宿主卡片跳转处理）。
